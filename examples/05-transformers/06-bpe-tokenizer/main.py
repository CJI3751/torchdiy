...
tokenizer.train(texts, num_merges=100)  # 限制合併次數以加快示例

# 測試分詞
test_text = "這是一個自然語言處理的測試。The brown fox is quick."
tokens = tokenizer.tokenize(test_text)
print(f"分詞結果: {tokens}")

# 編碼和解碼
token_ids = tokenizer.encode(test_text)
print(f"編碼: {token_ids}")

decoded_text = tokenizer.decode(token_ids)
print(f"解碼: {decoded_text}")

# 保存和加載
tokenizer.save_pretrained("./my_bpe_tokenizer")
loaded_tokenizer = BPETokenizer.from_pretrained("./my_bpe_tokenizer")

# 批量處理
batch = ["短文本", "這是一個更長的示例文本來演示填充"]
encoded_batch = tokenizer.batch_encode_plus(batch, max_length=15, padding=True, truncation=True)
print(encoded_batch)